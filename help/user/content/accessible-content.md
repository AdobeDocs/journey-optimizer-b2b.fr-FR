...
En [!DNL Journey Optimizer B2B Edition], la taille et l’espacement des différents éléments de votre contenu peuvent être affinés à l’aide des paramètres et attributs de style du volet Espace de conception visuel **[!UICONTROL Styles]**.
...
Pour un texte secondaire efficace dans les produits numériques, suivez ces directives :
...
Dans l’espace de conception visuelle, utilisez les composants pertinents [structure](structure-components.md) et [contenu](content-components.md) pour organiser votre contenu d’une manière claire, logique et concise, accessible à tous. Définissez les options du volet **[!UICONTROL Styles]** à l’aide des bonnes pratiques en matière d’héritage structurel.

![Espace de conception d’e-mail présentant la structure et les composants de contenu pour les dispositions organisées](assets/accessible-components.png){width="700" zoomable="yes"}
...
* Évitez l&#39;argot, le jargon ou les mots régionaux que certaines personnes ne connaissent peut-être pas.

Pour évaluer la lisibilité de votre contenu, utilisez le test populaire [Flesch Reading Ease](https://support.microsoft.com/fr-fr/office/get-your-document-s-readability-and-level-statistics-85b4969e-e80a-4777-8dd3-f7fc3c8b3fd2){target="_blank"} dans Microsoft Word. Il calcule la facilité de lecture de votre contenu sur une échelle de 0 à 100.

## Tester votre contenu {#test}

Pour vérifier l’accessibilité du contenu, vous pouvez utiliser les fonctionnalités de test fournies par [!DNL Journey Optimizer B2B Edition]. Elles ne sont pas spécialement conçues pour vérifier si le contenu est entièrement accessible, mais elles peuvent fournir un premier niveau de vérification.
...
Pour vérifier si votre contenu est accessible de manière fiable et plus cohérente, utilisez des outils externes tels que :
...
![Paramètres de conception pour le mode sombre](assets/email-color-mode-dark-text-settings.png){width="700" zoomable="yes"}

Pour la conception en mode sombre, utilisez des images PNG ou SVG transparentes et définissez les balises méta et CSS appropriées. Fournissez un style de secours accessible si le mode sombre n’est pas pris en charge. Enfin, testez tous les éléments de contenu et d’interface utilisateur en modes clair et sombre.
...
« Bonjour le monde. Bienvenue dans cette newsletter. » *(Aucune mention de lignes, colonnes ni tableau)*
...
### Texte des liens et des boutons {#links}
...
Utilisez les outils de conception pour ajouter un lien à votre contenu et modifier le libellé afin de le rendre perceptible (visible) et descriptif (clarifier l’objectif). Évitez les libellés vagues tels que _ici_ ou _plus_.

![Panneau Paramètres de lien affichant le champ URL et les options de libellé descriptif](assets/accessible-link.png){width="700" zoomable="yes"}
