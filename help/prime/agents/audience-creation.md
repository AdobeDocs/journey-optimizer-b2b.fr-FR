...
description: Utilisez les compétences de création d’audience dans Journey Optimizer B2B Prime pour créer des listes de personnes, adapter les listes dynamiques Marketo Engage et modifier les règles de liste dans le chat.
...
* **Lit à partir de / écrit sur** - [!DNL Journey Optimizer B2B Prime] ; lit les [!DNL Marketo Engage] lors de l’adaptation des listes dynamiques

## Workflows pris en charge {#workflows}

L’assistant AI prend en charge trois workflows de création d’audience et détermine celui qui s’applique à partir de votre requête. Si votre intention est ambiguë, il vous demande avant de continuer.
...
Avant de générer quoi que ce soit, l’assistant d’IA confirme les quatre éléments suivants. Il demande toutes les données manquantes dans un seul message.

1. **Règles/critères** — Description en langage clair de la personne qui fait partie de la liste.
1. **Nom** — Comment appeler la liste.
1. **Emplacement** — Dans quel programme la liste doit se trouver. Indiquez un nom de programme que l’assistant AI trouve ; s’il existe plusieurs correspondances, il vous demande de sélectionner.
1. **Type** : dynamique (basée sur des filtres, mise à jour automatique) ou statique (appartenance fixe). Obligatoire : l’assistant AI ne devine pas ; si vous ne spécifiez pas, il demande.

### Listes dynamiques {#dynamic-lists}

Pour les listes dynamiques, AI Assistant suggère de manière proactive d’inclure des attributs de personnalisation pour enrichir le ciblage. Ces attributs sont **_inclus par défaut — vous vous désinscrivez, pas dans_** :
...
Indiquez à l’assistant AI si vous souhaitez supprimer l’un de ces éléments avant de continuer.
...
* **Statique à partir de critères (instantané)** — L’assistant AI crée l’ensemble correspondant et copie ces personnes dans. La population est asynchrone — L’assistant AI confirme que la liste a été créée, mais remarque que l’affichage des personnes peut prendre quelques minutes. Il ne prétendra pas que la liste est prête immédiatement.

## Vérifier la carte {#review-card}

Rien n’est créé tant que vous ne l’avez pas approuvé. Après avoir décrit vos critères, l’assistant AI présente une vignette interactive _Révision de la création de la liste de personnes_ (pour les adaptations à partir de listes de [!DNL Marketo Engage], la vignette est intitulée _Révision de la conversion de la liste de personnes_).
...
Lorsque vous décrivez des critères, l’assistant AI convertit chaque condition en un attribut réel connu au niveau de la personne. Trois résultats peuvent apparaître sur la carte Révision :
...
Lorsque vous demandez à modifier des règles sur une liste que vous avez déjà, l’assistant AI détermine la liste et le mode de modification :
...
L’assistant AI résume ce qui sera appliqué et indique clairement s’il ajoute ou remplace, puis vous demande de confirmer avant sa validation. Après l’application, il indique le nombre total de règles et le nombre de règles ajoutées ou remplacées.
...
| **Taille de la liste** | Lit jusqu’à environ 1 000 premiers membres de chaque liste. Pour les listes plus volumineuses, AI Assistant indique que les résultats sont partiels. |
| **Brouillons de listes dynamiques** | Comparaison impossible : une liste qui n’a pas été publiée ne comporte aucun segment actif. L’assistant AI vous demande de le publier en premier ou d’utiliser une liste statique à la place. |
...
| Limite | Détail |
|---|---|
| **Adaptation de liste statique depuis[!DNL Marketo Engage]** | Vous ne pouvez pas adapter une liste statique [!DNL Marketo Engage] (ou un e-mail ou une autre ressource qui n’est pas une ressource de filtre) en liste de personnes. Les listes statiques sont des identifiants de membre explicites qui ne peuvent pas être exprimés en tant que filtres ; l’assistant AI demande plutôt une liste dynamique ou une campagne dynamique. |
...
| **Logique AND/OR profondément imbriquée** | Une logique imbriquée complexe peut se réduire en AND/OR de niveau supérieur ; l’assistant d’IA le remarque lorsque cela se produit. |
| **Collisions de noms** | Non résolu automatiquement : si le nom est pris, l’assistant AI vous en demande un autre au lieu d’ajouter silencieusement un suffixe. |
| **Approbation requise** | L’assistant AI ne crée ou ne modifie pas de liste tant que vous n’avez pas cliqué sur **[!UICONTROL Continuer]**, confirmé ou donné une autorisation claire (_« approuvé »_, _« semble correct »_ _« compiler »_). |
