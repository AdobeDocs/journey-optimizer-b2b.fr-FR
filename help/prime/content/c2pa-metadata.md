...
description: Découvrez comment Adobe Journey Optimizer B2B Prime applique automatiquement les métadonnées C2PA aux images générées avec l’IA générative et ce que cela signifie pour votre contenu.
...
Les métadonnées C2PA sont conservées avec le fichier image. Lorsqu’une image qui a été générée ou modifiée avec l’IA générative est chargée vers ou exportée à partir de [!DNL Adobe Journey Optimizer B2B Prime], ses métadonnées C2PA sont conservées.
...
Lorsque vous utilisez l’IA générative pour créer une image pour le contenu de votre e-mail dans [!DNL Journey Optimizer B2B Prime], les métadonnées C2PA sont automatiquement jointes à l’image générée et aucune action n’est requise de votre part. Les outils d’IA générative produisent un élément de métadonnées C2PA combiné pour les variantes d’images avec des métadonnées existantes, y compris la source d’origine.

>[!NOTE]
>
>[!DNL Journey Optimizer B2B Prime] ne prend actuellement pas en charge les actions de modification manuelle d’images. Les workflows de métadonnées C2PA pour ces actions ne sont pas applicables à ce stade.
